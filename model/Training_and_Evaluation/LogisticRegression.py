...
def main():
    ...
    lr_importance_df = lr_importance_df.sort_values(by='abs_coefficient', ascending=False).reset_index(drop=True)
    ...
    plt.title('Feature Importance (Logistic Regression)', fontsize=15)
    ...
    plt.show()
    # Save model
    lr_model.write().overwrite().save("model/lr_spark_model")

    test_predictions = lr_model.transform(df_test)
    model_evaluation(lr_model, test_predictions, "Logistic Regression", has_probability=True)
